...
const ProjectComments: React.FunctionComponent<ProjectCommentsProps> = (
  props
) => {
  ...
  const deleteComment = async (projid: string, comid: string) => {
    if (props.project && props.project.comments) {
      const filteredComments = props.project.comments.filter(
        (comment) => comment.id.toString() !== comid
      );
              ...
              props.project.comments.map((com) => (
                <li key={com.id}>
                  <p className="comment-date">{`${format(
                    new Date(com.created_at?.toMillis()),
                    "yyyy-MM-dd HH:mm"
                  )}`}</p>
                  <div className="comment-author" data-comment-id={com.id}>
                    {com.uid && com.uid === currentUser?.uid && (
                      <img
                        src={dot}
                        width="20px"
